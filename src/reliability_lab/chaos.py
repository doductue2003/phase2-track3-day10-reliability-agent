...
def calculate_recovery_time_ms(gateway: ReliabilityGateway) -> float | None:
    ...
    for breaker in gateway.breakers.values():
        ...
        for entry in breaker.transition_log:
            ...
            ts = entry.get("ts")
            if to_state == "open" and isinstance(ts, (int, float)):
                last_open_ts = ts
            elif to_state == "closed" and isinstance(last_open_ts, (int, float)) and isinstance(ts, (int, float)):
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    for scenario in config.scenarios:
        ...
        if scenario.name == "primary_timeout_100":
            passed = result.availability >= 0.80 and result.fallback_success_rate >= 0.80
        elif scenario.name == "primary_flaky_50":
            passed = result.availability >= 0.80
        elif scenario.name == "all_healthy":
            passed = result.availability >= 0.98 and result.circuit_open_count == 0
        else:
            passed = result.availability >= 0.80
